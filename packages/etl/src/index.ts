...
async function fetchRektIncident(card: RektListingCard, fetchImpl: FetchLike): Promise<RektIncidentRecord> {
...
export async function fetchRekt2026Incidents(options: { fetchImpl?: FetchLike; maxPages?: number } = {}): Promise<RektIncidentRecord[]> {
...
export async function fetchSoloditFindings(options: { fetchImpl?: FetchLike; maxFindings?: number; maxSitemaps?: number } = {}): Promise<SoloditFindingRecord[]> {
  const fetchImpl = options.fetchImpl ?? fetch;
