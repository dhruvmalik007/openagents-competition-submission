...
function decodeHtmlEntities(value) {
    return value.replace(HTML_ENTITY_PATTERN, (_match, entity) => {
...
function parseUsdEstimate(value) {
...
function parseRektListing(html) {
    const cards = [];
...
async function fetchRektIncident(card, fetchImpl) {
    const html = await fetchText(card.sourceUrl, fetchImpl);
...
export async function fetchRekt2026Incidents(options = {}) {
    ...
    const incidents = await Promise.all(Array.from(cardsBySlug.values()).map((card) => fetchRektIncident(card, fetchImpl)));
...
function extractIssueLocs(xml, maxCount) {
    if (maxCount <= 0) {
        return [];
    }
    const matches = [];
...
function parseSoloditSeverity(slug) {
...
function inferSoloditMetadata(slug) {
...
export async function fetchSoloditFindings(options = {}) {
    const fetchImpl = options.fetchImpl ?? fetch;
    const maxFindings = options.maxFindings ?? 2500;
    const maxSitemaps = options.maxSitemaps ?? 8;
    const sitemapIndex = await fetchText(SOLODIT_SITEMAP_INDEX, fetchImpl);
    ...
    for (const sitemapUrl of sitemaps) {
        ...
        const payload = await fetchText(sitemapUrl, fetchImpl);
...
export function build0GModuleChecks(input) {
